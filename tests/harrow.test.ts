...
test("building the Harrow changes nothing anywhere else, on any plane, and leaves the neighbours' north rows alone", () => {
  ...
  assert.equal(builtRegions(alone).length, 88, "the control build is everything but the Harrow");
  const theirs = (o: { x: number; y: number; plane: number }) => !inBox(HARROW, o.x, o.y) && !(o.plane < 0 && (inBox(ADIT_REGION, o.x, o.y) || inBox(DEEP_REGION, o.x, o.y)));
  for (const [plane, before] of without.planes) {
    ...
    assert.deepEqual(after.monsters.filter((s) => !inBox(HARROW, s.x, s.y)), before.monsters.filter((s) => !inBox(HARROW, s.x, s.y)), `plane ${plane}: and their creatures`);
  }
