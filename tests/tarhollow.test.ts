...
test("building the isle changes nothing anywhere else, on any plane", () => {
  ...
  assert.equal(builtRegions(without.planes.get(0)!).length, 191, "the control build is everything but the isle");
  const theirs = (o: { x: number; y: number; plane: number }) => !inBox(SABLEWOOD, o.x, o.y) && !inBox(DEEPDELVE_SITE, o.x, o.y) && !inBox(HARROW, o.x, o.y) && !inBox(SANDREACH_SITE, o.x, o.y) && !inFenSites(o.x, o.y) && !(o.plane < 0 && (inBox(ADIT_REGION, o.x, o.y) || inBox(DEEP_REGION, o.x, o.y)));
  for (const [plane, before] of without.planes) {
    const after = stack.planes.get(plane)!;
    for (const r of builtRegions(before).filter((r) => !inBox(ADIT_REGION, r.x0, r.y0) && !inBox(DEEPDELVE_SITE, r.x0, r.y0) && !inBox(HARROW, r.x0, r.y0) && !inBox(SANDREACH_SITE, r.x0, r.y0) && !inFenSites(r.x0, r.y0))) {
    ...
    assert.deepEqual(after.monsters.filter((s) => !inBox(SABLEWOOD, s.x, s.y) && !inBox(DEEPDELVE_SITE, s.x, s.y) && !inBox(HARROW, s.x, s.y) && !inBox(SANDREACH_SITE, s.x, s.y) && !inFenSites(s.x, s.y)), before.monsters.filter((s) => !inBox(SABLEWOOD, s.x, s.y) && !inBox(DEEPDELVE_SITE, s.x, s.y) && !inBox(HARROW, s.x, s.y) && !inBox(SANDREACH_SITE, s.x, s.y) && !inFenSites(s.x, s.y)), `plane ${plane}: and their creatures`);
    assert.deepEqual(after.spawns.filter((s) => !inBox(SABLEWOOD, s.x, s.y) && !inBox(DEEPDELVE_SITE, s.x, s.y) && !inBox(HARROW, s.x, s.y) && !inBox(SANDREACH_SITE, s.x, s.y) && !inFenSites(s.x, s.y)), before.spawns.filter((s) => !inBox(SABLEWOOD, s.x, s.y) && !inBox(DEEPDELVE_SITE, s.x, s.y) && !inBox(HARROW, s.x, s.y) && !inBox(SANDREACH_SITE, s.x, s.y) && !inFenSites(s.x, s.y)), `plane ${plane}: and what lies about`);
  }
