...
const theirs = (x: number, y: number) => !inBox(KILNHOLD_SITE, x, y) && !inBox(SABLEWOOD, x, y) && !inBox(DEEPDELVE_SITE, x, y) && !inBox(HARROW, x, y);
...
test("building Kilnhold changes nothing in the district or the sites of Wave 1, on any plane", () => {
  ...
  assert.equal(builtRegions(alone).length, 121, "the control build is everything but the hold");
  ...
  for (const [plane, before] of without.planes) {
    ...
    for (const r of builtRegions(before).filter((r) => !inBox(SABLEWOOD, r.x0, r.y0) && !inBox(ADIT_REGION, r.x0, r.y0) && !inBox(DEEPDELVE_SITE, r.x0, r.y0) && !inBox(HARROW, r.x0, r.y0))) {
...
test("the people of Kilnhold talk, the shop keeper's talk offers the trade, and the map knows the hold", () => {
  ...
  assert.ok(MAP_EXITS.some((e) => e.name.includes("Sand Road") && e.x === SAND_EXIT.x && e.side === "e"), "the Sand Road leaves east");
